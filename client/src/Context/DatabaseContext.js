  ...
  const handleTokenChange = () => {
    patients.forEach((patient) => {
      if (
        ...
        patient.token !== patients[patients.length - 1].token
      ) {
        setCurrToken(`T-${Number(patient.token.substring(2, 5)) + 1}`);
        socket.emit(
          "tokenChange",
          `T-${Number(patient.token.substring(2, 5)) + 1}`
        );
      }
    });
  };
    ...
    try {
      if (name.length < 50 && name.length > 1) {
        if (patients.length > 0) {
          setToken(
            Number(patients[patients.length - 1].token.substring(2, 5)) + 1
          );
          // setToken((prev) => prev + 2);
        }
